    ...
    function initDynamic() {
        ...
        $('.sample-request-content-type-switch').change(function () {
            if ($(this).val() === 'body-form-data') {
                $(
                    '#sample-request-body-json-input-' + $(this).data('id'),
                ).hide()
                $(
                    '#sample-request-body-form-input-' + $(this).data('id'),
                ).show()
            } else {
                $(
                    '#sample-request-body-form-input-' + $(this).data('id'),
        ...
        if (apiProject.template.aloneDisplay) {
            ...
            if (hashVal != null && hashVal.length !== 0) {
                ...
                const el = document.querySelector(
                    `li .${hashVal.slice(1)}-init`,
                )
                const elVersioned = document.querySelector(
                    `li[data-version="${version}"] .show-api.${hashVal.slice(
                        1,
                    )}-init`,
                )
                let targetEl = el
